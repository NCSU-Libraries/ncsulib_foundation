...
<div class="calendar-calendar test"><div class="day-view">
        ...
        <td class="first">
          ...
          <?php foreach ($rows['items'] as $time_cnt => $hour): ?>
            ...
            <div class="<?php print $class?>calendar-agenda-hour">
              ...
              ?>
              <span class="calendar-hour"><?= $hr ?></span><span class="calendar-ampm"><?php print $hour['ampm']; ?></span>
            </div>
          <?php endforeach; ?>
        </td>
        <td class="last">
              ...
              <div class="inner">
               ...
               <?php if (!empty($hour['values']) && is_array($hour['values']) && array_key_exists($column, $hour['values'])): ?>
                 <?php foreach ($hour['values'][$column] as $item): ?>
                    <div class="<?php echo 'size-'.floor(100/count($hour['values'][$column])); ?>">
                   <?php print $item; ?>
                    </div>
                 <?php endforeach; ?>
               <?php else: ?>
